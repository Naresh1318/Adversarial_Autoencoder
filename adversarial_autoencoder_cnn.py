...
real_dist = tf.placeholder(tf.float32, [None, 100], name='real_distribution')
...
def discriminator(x, reuse=False):
    if reuse:
        tf.get_variable_scope().reuse_variables()
    dc_den1 = tf.nn.dropout(tf.nn.relu(dense(x, 100, 500, name='dc_den1')), keep_prob=.25)
    dc_den2 = tf.nn.dropout(tf.nn.relu(dense(dc_den1, 500, 500, name='dc_den2')), keep_prob=.25)
    output = tf.nn.dropout(tf.nn.sigmoid(dense(dc_den2, 500, 1, name='dc_output')), keep_prob=.25)
    return output
